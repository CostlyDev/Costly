...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, mempoolrej, net, proxy, prune, http, libevent, tor, zmq, "
                             "costly (or specifically: privatesend, instantsend, masternode, spork, keepass, mnpayments, gobject)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Costly specific functionality (Masternodes, PrivateSend, InstantSend, Governance) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-privatesendamount=<n>", strprintf(_("Keep N COSTLY anonymized (default: %u)"), DEFAULT_PRIVATESEND_AMOUNT));
...
/** Sanity checks
 *  Ensure that Costly Core is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Costly Core version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
}

/** Initialize Costly Core.
...
#define PROCESS_DEP_ENABLE 0x00000001
...
#endif
    // Make sure only a single Costly Core process is using the data directory.
    ...
    try {
        ...
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Costly Core is probably already running.") + " %s.", strDataDir, e.what()));
    }
