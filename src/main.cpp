...
#if defined(NDEBUG)
# error "Costly Core cannot be compiled without assertions."
#endif
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    if (nPrevHeight >= 100 && nPrevHeight < 4100){
        nSubsidy = 0.01 * COIN;
    }else if (nPrevHeight >= 4100 && nPrevHeight < 30000){
        nSubsidy = 5 * COIN;
    }else if (nPrevHeight >= 30000 && nPrevHeight < 60000){
        nSubsidy = 7.5 * COIN;
    }else if (nPrevHeight >= 60000 && nPrevHeight < 100000){
        nSubsidy = 10 * COIN;
    }else if (nPrevHeight >= 100000 && nPrevHeight < 150000){
        nSubsidy = 15 * COIN;
    }else if (nPrevHeight >= 150000 && nPrevHeight < 300000){
        nSubsidy = 20 * COIN;
    }else if (nPrevHeight >= 300000 && nPrevHeight < 350000){
        nSubsidy = 25 * COIN;		
    }
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // COSTLY : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(COSTLY): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(COSTLY): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // COSTLY : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(COSTLY): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(COSTLY): spork is off, skipping transaction locking checks\n");
    }
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /* 
        Costly Related Inventory Messages
