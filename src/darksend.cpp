...
/* *** BEGIN DARKSEND MAGIC - CST **********
    Copyright (c) 2014-2015, Costly Developers
        eduffield - evan@costly.org
        udjinm6   - udjinm6@costly.org
*/
    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Costly and make it unusable. To
...
bool CDarksendPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeCostlyAmount*COIN - pwalletMain->GetAnonymizedBalance();
...
{
    if(fLiteMode) return; // disable all Costly specific functionality
    ...
    RenameThread("costly-privatesend");
