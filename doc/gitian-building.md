...
*Setup instructions for a Gitian build of Costly using a Debian VM or physical system.*
...
- [Building Costly](#building-costly)
...
Clone the git repositories for costly and Gitian.
...
in the costly repository under 'Fetch and build inputs' to install sources which require
...
To build Costly (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the costly repository.
...
and the costly git repository with the desired tag must both be available locally, and then gbuild must be
