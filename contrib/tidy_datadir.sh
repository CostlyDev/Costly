...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Costly database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Costly datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Costly datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Costly 0.7 datadir."
    ;;
  3)
    echo "Detected Costly pre-0.8 datadir."
    ;;
  4)
    echo "Detected Costly 0.8 datadir."
    ;;
esac
