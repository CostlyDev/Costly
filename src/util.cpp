...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "costly" is a composite category enabling all Costly-related debug output
            if(ptrCategory->count(string("costly"))) {
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty costly.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override costly.conf
